...
export function UsersPage() {
  ...
  const toggle = async (u: AdminUser) => { try { const next = await updateAdminUser(u.id, { status: u.status === "ACTIVE" ? "DISABLED" : "ACTIVE" }); setItems(xs => xs.map(x => x.id === u.id ? next : x)); } catch (err) { setError(err); } };
