...
export function listAdminUsers(params: { keyword?: string; status?: string } = {}) { const q = new URLSearchParams(); Object.entries(params).forEach(([k,v]) => v && q.set(k,v)); return apiGet<{items: AdminUser[]; total: number}>(`/api/v1/admin/users?${q}`); }
...
export function listAdminConversations(params: { keyword?: string } = {}) { const q = new URLSearchParams(); if (params.keyword) q.set("keyword", params.keyword); return apiGet<{items: AdminConversation[]; total: number}>(`/api/v1/admin/conversations?${q}`); }
